...
public class ListaGenerosDAO {
    ...
    public static List <bean.Generos> obterGenerosNaoAtribuidosAoUsuario (int id){
        String sql = "select idGeneros, genero "
                + " from tb_generos_has_tb_usuario "
                + " right join tb_generos "
                + " on tb_generos.idGeneros = tb_generos_has_tb_usuario.tb_generos_idGeneros "
                + " where tb_generos_has_tb_usuario.tb_usuario_idusuario <> ? "
                + " or tb_generos_has_tb_usuario.tb_usuario_idusuario is null";
        List <bean.Generos> generos = new ArrayList<>();
        try {
           Connection conexao = connection.ConexaoDB.obterConexao();
            PreparedStatement ps = conexao.prepareStatement(sql);
            ...
            while (rs.next()){
                ...
                String nome = rs.getString("genero");
                //String tipo = rs.getString("tipo");
                bean.Generos genero = new bean.Generos(idGeneros, nome);
                generos.add(genero);
                //cursos.add(new Curso (id, nome, tipo));
                //System.out.print("Passei nos campos");
            }
